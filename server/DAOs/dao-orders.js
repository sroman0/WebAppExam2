...
async function getOrdersByUser(userId) {
  const sql = `SELECT o.id, o.dish_id, d.name as dishName, o.size, o.total, o.date, o.cancelled
...
async function getOrderDetails(orderId) {
  const sql = `SELECT o.*, d.name as dishName FROM orders o JOIN dishes d ON o.dish_id = d.id WHERE o.id = ?`;
...
async function getOrderIngredients(orderId) {
  const sql = `SELECT i.id, i.name, i.price FROM order_ingredients oi JOIN ingredients i ON oi.ingredient_id = i.id WHERE oi.order_id = ?`;
  return await db.all(sql, [orderId]);
}

// Create a new order
async function createOrder(userId, dishId, size, total, ingredientIds) {
  const sql = `INSERT INTO orders (user_id, dish_id, size, total, date) VALUES (?, ?, ?, ?, datetime('now'))`;
